...
def evolve_gravity(bodies, converter, t_end, dt_integration, n_snapshots):
    ...
    masses = [607, 7.5, 3.3667] #in jupiter mass
    #bodies_filename = 'traj_m3.3667_withoutcheckingE.txt'
    bodies_filename = 'traj_m3.3667_600snapshots.txt'
    ...
    if(t_end != 0.0 | nbody_system.time):
        ...
        while (t <= t_end):
            ...
            print t, 'antes de evolve'
            
            gravity.evolve_model(t)
            channel_from_gr_to_framework.copy()
            
            print t, 'despues de evolve'
            
            x = converter.to_si(bodies.x-bodies.x[0]).value_in(units.AU)
            y = converter.to_si(bodies.y-bodies.y[0]).value_in(units.AU)
            vx = converter.to_si(bodies.vx-bodies.vx[0]).value_in(units.kms)
            vy = converter.to_si(bodies.vy-bodies.vy[0]).value_in(units.kms)
            
            to_write = str(converter.to_si(t).value_in(units.yr))+'\t3\t'+'0.0'
            os.system('echo '+to_write+' >> '+bodies_filename)
            for i in range(3):
                to_write = str(masses[i])+'\t'+str(x[i])+'\t'+str(y[i])+'\t'+'0.0'+'\t'+str(vx[i])+'\t'+str(vy[i])+'\t'+'0.0'
                os.system('echo '+to_write+' >> '+bodies_filename)
            to_write = ' '
            os.system('echo '+to_write+' >> '+bodies_filename)
