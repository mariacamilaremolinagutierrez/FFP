...
def get_bodies_in_orbit(m0, m_bp, e_bp, a_bp, m_ffp, v_ffp, b_ffp, phi_bp, inc_bp, lan_bp, ap_bp, r_inf):
    ...
    ##Get BP in orbit
    #Binary
    star_planet = new_binary_from_orbital_elements(m0, m_bp, a_bp, e_bp, true_anomaly=phi_bp, inclination = inc_bp, longitude_of_the_ascending_node = lan_bp, argument_of_periapsis = ap_bp)
    #Planet attributes
    ...
    m0_ffp[1].velocity = (v_ffp+cm_v[0], cm_v[1], cm_v[2])
...
def convert_units(converter, m0_p, m_bp_p, e_bp_p, a_bp_p, m_ffp_p, v_ffp_p, b_ffp_p, phi_bp_p, inc_bp_p, lan_bp_p, ap_bp_p):
    ...
    #velocity of the incoming ffp
    v_ffp = converter.to_nbody(v_ffp_p | units.kms)
    ...
    return m0, m_bp, e_bp, a_bp, m_ffp, v_ffp, b_ffp, phi_bp, inc_bp, lan_bp, ap_bp

def run_capture(m0_p=1.0, m_bp_p=1.0, e_bp_p=0.0, a_bp_p=5.20336301, m_ffp_p=1.0, v_ffp_p=50.0, b_ffp_p=1.0, phi_bp_p=0.0, inc_bp_p=0.0, lan_bp_p=0.0, ap_bp_p=0.0, n_snapshots=600, n_r0_in_rinf=40.0):
    """
    Units: m0_p(MSun), m_bp_p(MJupiter), e_bp_p(None), a_bp_p(AU), m_ffp_p(MJupiter), v_ffp_p(km/s), b_ffp(AU), phi_bp_p(degrees), inc_bp_p(degrees), lan_bp_p(degrees), ap_p(degrees), n_snapshots(None), n_r0_in_rinf(None)
    """
    #Converter used in this program
    converter = nbody_system.nbody_to_si(1 | units.MSun,  5 | units.AU)

    #Conversion of units
    m0, m_bp, e_bp, a_bp, m_ffp, v_ffp, b_ffp, phi_bp, inc_bp, lan_bp, ap_bp = convert_units(converter, m0_p, m_bp_p, e_bp_p, a_bp_p, m_ffp_p, v_ffp_p, b_ffp_p, phi_bp_p, inc_bp_p, lan_bp_p, ap_bp_p)

    #Initial distance to the planet (x-cordinate)
    r_inf = n_r0_in_rinf*a_bp

    #Particles: star, FFP, planets
    bodies, time_pericenter, orbital_period_bp = get_bodies_in_orbit(m0, m_bp, e_bp, a_bp, m_ffp, v_ffp, b_ffp, phi_bp, inc_bp, lan_bp, ap_bp, r_inf)
